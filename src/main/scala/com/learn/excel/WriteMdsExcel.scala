...
class WriteMdsExcel {
					...
					data._2.map(ep => {
						...
						Row(ep.number) {
							ep.number match {
								case 0 => Set(StringCell(0, "订单编号"), StringCell(1, "医生姓名"), StringCell(2, "患者姓名"), StringCell(3, "患者性别"), StringCell(4, "患者年龄"),StringCell(5, "诊断"),
									StringCell(6, "药品编码"), StringCell(7, "通用名"),StringCell(8,"药品名称"), StringCell(9, "购买数量"), StringCell(10, "频次"),
									StringCell(11, "单次使用量"), StringCell(12, "规格"), StringCell(13, "生产厂家"), StringCell(14, "规格大小"), StringCell(15, "违反规则"), StringCell(16, "可疑规则"))
								case _ => Set(StringCell(0, ep.orderNo), StringCell(1, ep.doctorName), StringCell(2, ep.patientName), StringCell(3, ep.patientSex),
									NumericCell(4, ep.patientAge.toDouble),StringCell(5, ep.orderDiseases),
									StringCell(6, ep.drugCode), StringCell(7, ep.commonName),StringCell(8, ep.productName), NumericCell(9, ep.buyAmount.toDouble),
									NumericCell(10, ep.frequency.toDouble), NumericCell(11, ep.useAmount.toDouble), StringCell(12, ep.standard),StringCell(13, ep.producer), NumericCell(14, ep.packageSize), StringCell(15, handleRules(ep.orderNo, ep.rules)), StringCell(16, handleDubious(ep.orderNo, ep.dubious)))
							}
						}
					})
	...
	var setd:scala.collection.mutable.Set[String] = scala.collection.mutable.Set("1")
	def handleDubious(orderNo: String, rules: String):String = {
		if(!setd.contains(orderNo)) {
			setd = setd + orderNo
			var index = 1
			var result = ""
			for (rule <- rules.split(";:")) {
...
object WriteMdsExcel extends App{
	val path = "/Users/zcx/scalaExcelTest/mdy5.xls";
